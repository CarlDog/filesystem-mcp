...
The dev chat picking this up should read [HANDOFF.md](HANDOFF.md) first
— it's the briefing for what's done, what's decided, and what's next.
...
│   │   ├── read.ts             # read tools (list_roots, list_directory, stat — implemented; read_file, find_by_glob — stubs)
│   │   └── write.ts            # write tools (move, copy, delete, mkdir — all stubs; only registered when FS_ALLOW_WRITE=true)
│   └── util.ts                 # asText() helper
├── docs/                       # specs / refs (empty in scaffold)
├── tests/                      # empty in scaffold
...
└── (CLAUDE.md, STATUS.md, README.md, HANDOFF.md)
```
